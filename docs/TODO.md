...
- [x] **Plugin hook system** — Three-point hook system (`pre_tool_call`, `post_tool_call`, `filter_tools`) with config-driven rules in `radar.yaml` and plugin `hook` capability. Supports blocking, observing, and filtering tools. (51 tests in `test_hooks.py`)
